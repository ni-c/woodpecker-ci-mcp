...
features:
  ...
  - title: Safe by default
    details: 'Twenty-three irreversible operations ask a person first, through MCP elicitation — a dialog the model cannot answer on its behalf, falling back to a server-issued token bound to the exact arguments where the client cannot show one. Agent tokens are redacted on read, build logs are marked untrusted, and read-only mode simply does not register the write tools.'
---
